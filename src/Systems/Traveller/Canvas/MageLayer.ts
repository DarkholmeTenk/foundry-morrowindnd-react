...
export default class MageLayer extends PIXI.Container {
    ...
    draw() {
        ...
        if (mages.length > 1) {
            let center = mages.map(n => n.note.data).reduce((p, c) => ({ x: p.x + (c.x / mages.length), y: p.y + (c.y / mages.length) }), { x: 0, y: 0 });
            ...
            mages.forEach(mage => {
                this.addChild(new CircleLine(mage.note.data, center, style))
            });
        }
    }
}