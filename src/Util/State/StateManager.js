...
export function SetupReducers(name, defaultState) {
    let Constr = defaultState.constructor;
    let set = createAction("set");
    const actThunk = createAsyncThunk("act", async (payload, thunkAPI) => {
...
export function SetupStateListener(name, store) {
    let { actThunk, set } = reducers[name];
    let actSocket = registerGMSocket(`SM_Act_${name}`, (data) => {
        console.log("Acting", data);
        store.dispatch(actThunk(data));
    });
    let setSocket = registerPlayerSocket(`SM_Set_${name}`, (data) => {
        console.log("Setting", data);
        store.dispatch(set({ value: data }));
    });
    let refreshSocket = registerGMSocket(`SM_Refresh_${name}`, () => {
