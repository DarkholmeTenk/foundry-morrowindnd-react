type Decorator = (target: any, propertyKey: string, descriptor: any)=>any

export function Updatable(target, propertyKey, descriptor) {
    let originalMethod = descriptor.value;
    let method = function () {
        let result = originalMethod.bind(this)(...arguments)
        if(this._updateFunction) this._updateFunction(result)
        return result
    };
    return {
        ...descriptor,
...
interface UpdatableProviderObj {
    ArrayElement: (fieldName: string) => Decorator,
    ObjectField: (fieldName: string) => Decorator,
    Replace: (fieldName: string) => Decorator,
    Setter: Decorator
}

export function UpdatableProvider(onUpdate): UpdatableProviderObj {
    let getDecorator = (updater: (x: any, result: any, args: any)=>void) => {
        return function(target, propertyKey, descriptor) {
            ...
            if(descriptor.getter) newDescriptor.getter = newFunction
            if(descriptor.value) newDescriptor.value = newFunction
